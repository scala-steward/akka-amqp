...
class QueueDeclarationMode private[amqp] (val name: String) {
  ...
  )           = ActiveUndeclaredQueue(name, durable, exclusive, autoDelete, arguments)
...
class DefaultQueueDeclarationMode private[amqp] {
  def active(
      ...
      autoDelete: Boolean = true,
      arguments: Option[Map[String, AnyRef]] = None
  )           = ActiveUndeclaredQueue("", durable, exclusive, autoDelete, arguments)
...
case class QueueParams(
    ...
    exclusive: Boolean,
    autoDelete: Boolean,
    arguments: Option[Map[String, AnyRef]]
)
case class ActiveUndeclaredQueue private[amqp] (
    name: String,
    durable: Boolean,
    exclusive: Boolean,
    autoDelete: Boolean,
...
case class PassiveUndeclaredQueue private[amqp] (name: String) extends Queue with UndeclaredQueue {
  def nameOption = if (name.size != 0) Some(name) else None
  def params     = None
  def declare(implicit channel: RabbitChannel, system: ActorSystem): DeclaredQueue = {
...
case class DontDeclareUndeclaredQueue private[amqp] (name: String) extends Queue with UndeclaredQueue {
  def nameOption = if (name.size != 0) Some(name) else None
  def params     = None
  def declare(implicit channel: RabbitChannel, system: ActorSystem): DeclaredQueue = {
...
case class DeclaredQueue(peer: RabbitQueue.DeclareOk, params: Option[QueueParams], val undeclared: UndeclaredQueue)
    extends Queue {
...
trait QueueBinding extends Declarable[DeclaredQueueBinding] {
  ...
  protected def doDeclare(
      arguments: Option[Seq[(String, AnyRef)]],
      undeclared: QueueBinding
  )(implicit channel: RabbitChannel, system: ActorSystem): DeclaredQueueBinding = {
    val q = declaredQueue
...
case class DeclaredQueueBinding(
    ok: Option[RabbitQueue.BindOk],
    queue: DeclaredQueue,
    exchange: DeclaredExchange,
    undeclared: QueueBinding
)
...
case class QueueBinding2(exchange: Exchange, queue: Queue, routingKey: String, arguments: (String, AnyRef)*)
